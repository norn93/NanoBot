...
long _Nano_left_micros; //for the PID stuff later
long _Nano_left_micros_last; //for the PID stuff later
float _Nano_left_speed; //for the PID stuff later
...
void NanoInitialise() {
  ...
  _NanoInitialiseOptocouplers();

  //initialise the interrupts
  _NanoInitialiseTimerInterrupts();
}
...
//these next 2 functions will eventually be replaced by ones with closed loops, but these will do for now.
void NanoForward(float speed, int duration) {
  NanoDrive(speed, speed);
  delay(duration);
  NanoStop();
}
...
void _NanoMotorTest() {
  //a function for testing the motors
  NanoForward(1, 1000);
  NanoBackward(1, 1000);
  NanoTurn(1, 100, 'l');
  NanoTurn(1, 100, 'r');
